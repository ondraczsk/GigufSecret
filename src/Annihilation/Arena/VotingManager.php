...
class VotingManager{
    ...
    public $allVotes = ['Canyon', 'Amazon', 'Hamlet', 'Cavern', 'Planities', /*'Andorra', 'Kingdoms',*/ 'Solumque', 'Cliffs']; //nektere mapy se buguji
    public $currentTable = [];
    public $stats = [];
    ...
    public function onVote(Player $p, $vote){
        ...
        if(is_numeric($vote)){
            if(!(intval($vote) >=1 || intval($vote) <= 3)){
                $p->sendMessage($this->plugin->plugin->getPrefix().TextFormat::GRAY."pouzij /vote [map]");
                return;
            }
            ...
            $p->sendMessage($this->plugin->plugin->getPrefix().TextFormat::YELLOW."Voted for ".$this->currentTable[intval($vote)-1]);
            return;
        }
        if(is_string($vote)){
            ...
            $p->sendMessage($this->plugin->plugin->getPrefix().TextFormat::GOLD."Hlasoval jsi pro mapu ".$vote);
            return;
        }
    }
}
